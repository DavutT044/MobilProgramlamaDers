...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnConfirm.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                String kullaniciGirdisi = etCity.getText().toString().trim();
                String dogruSehir = sehirler[plaka][1];

                if (kullaniciGirdisi.equalsIgnoreCase(dogruSehir)) {
                    System.out.println("Onaylandı !");
                    Intent intent = new Intent(MainActivity.this, ekran2.class);
                    intent.putExtra("veriAnahtari",etCity.getText());
                    startActivity(intent);
                } else {
                    System.out.println("Onaylanmadı! Lütfen girdiğiniz değeri kontrol edin...");
                }
                long endTime = System.currentTimeMillis();
                long elapsedTime = endTime - time;
                Intent intent2 = new Intent(MainActivity.this, ekran2.class);
                intent2.putExtra("time",elapsedTime);
            }
        });
    }
}